...
template<>
InputParameters validParams<stzanaFiniteStrainUObasedCPCouple>()
{
...
stzanaFiniteStrainUObasedCPCouple::stzanaFiniteStrainUObasedCPCouple(const InputParameters & parameters) :
    ...
    _sdplcouple(declareProperty<Real>("sdplcouple")), // SDPL_TERM COUPLING
...

{
  ...
  {
    _uo_slip_rates[i] = &getUserObjectByName<stzanaCrystalPlasticitySlipRateGSSCouple>(parameters.get<std::vector<UserObjectName> >("uo_slip_rates")[i]);
  ...
  {
    _uo_slip_resistances[i] = &getUserObjectByName<stzanaCrystalPlasticitySlipResistanceGSSCouple>(parameters.get<std::vector<UserObjectName> >("uo_slip_resistances")[i]);
  ...
  {
    _uo_state_vars[i] = &getUserObjectByName<stzanaCrystalPlasticityStateVariableCouple>(parameters.get<std::vector<UserObjectName> >("uo_state_vars")[i]);
  ...
  {
    _uo_state_var_evol_rate_comps[i] = &getUserObjectByName<stzanaCrystalPlasticityStateVarRateComponentGSSCouple>(parameters.get<std::vector<UserObjectName> >("uo_state_var_evol_rate_comps")[i]);
...
void stzanaFiniteStrainUObasedCPCouple::initQpStatefulProperties()
{
  for (unsigned int i = 0; i < _num_uo_slip_rates; ++i)
  {
 ...
 */
void stzanaFiniteStrainUObasedCPCouple::computeQpStress()
{
...
void
stzanaFiniteStrainUObasedCPCouple::preSolveQp()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
stzanaFiniteStrainUObasedCPCouple::solveQp()
{
  preSolveStatevar();
  solveStatevar();
  if (_err_tol)
...
void
stzanaFiniteStrainUObasedCPCouple::postSolveQp()
{
...
// Calculating v1*tau1+v2*tau2+...

  _sdplcouple[_qp] = 2.0*(*_mat_prop_slip_rates[0])[_qp][0]*_tau1[_qp]+ \
                     2.0*(*_mat_prop_slip_rates[0])[_qp][2]*_tau2[_qp]+\
                     2.0*(*_mat_prop_slip_rates[0])[_qp][4]*_tau2[_qp];
...
void
stzanaFiniteStrainUObasedCPCouple::preSolveStatevar()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
stzanaFiniteStrainUObasedCPCouple::solveStatevar()
{
  unsigned int iterg;
  bool iter_flag = true;
...
bool
stzanaFiniteStrainUObasedCPCouple::isStateVariablesConverged()
{
  Real diff;
...
void
stzanaFiniteStrainUObasedCPCouple::postSolveStatevar()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
    (*_mat_prop_state_vars_old[i])[_qp] = (*_mat_prop_state_vars[i])[_qp];
...
void
stzanaFiniteStrainUObasedCPCouple::preSolveStress()
{
}

void
stzanaFiniteStrainUObasedCPCouple::solveStress()
{
  unsigned int iter = 0;
  RankTwoTensor dpk2;
...
void
stzanaFiniteStrainUObasedCPCouple::postSolveStress()
{
  _fp[_qp] = _fp_inv.inverse();
}

void
stzanaFiniteStrainUObasedCPCouple::updateSlipSystemResistanceAndStateVariable()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
stzanaFiniteStrainUObasedCPCouple::calcResidJacob()
{
  calcResidual();
  if (_err_tol)
    return;
...
void
stzanaFiniteStrainUObasedCPCouple::getSlipRates()
{
  for (unsigned int i = 0; i < _num_uo_slip_rates; ++i)
  {
...
void
stzanaFiniteStrainUObasedCPCouple::calcResidual()
{
...
void
stzanaFiniteStrainUObasedCPCouple::calcJacobian()
{
...
void
stzanaFiniteStrainUObasedCPCouple::calcTangentModuli()
{
...
void
stzanaFiniteStrainUObasedCPCouple::elastoPlasticTangentModuli()
{
...
void
stzanaFiniteStrainUObasedCPCouple::elasticTangentModuli()
{
...
bool
stzanaFiniteStrainUObasedCPCouple::lineSearchUpdate(const Real rnorm_prev, const RankTwoTensor dpk2)
{
