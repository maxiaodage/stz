...
template<>
InputParameters validParams<anaFiniteStrainUObasedCP>()
{
...
anaFiniteStrainUObasedCP::anaFiniteStrainUObasedCP(const InputParameters & parameters) :
    ...
    _crysrot(getMaterialProperty<RankTwoTensor>("crysrot"))
{
  ...
  {
    _uo_slip_rates[i] = &getUserObjectByName<anaCrystalPlasticitySlipRateGSS>(parameters.get<std::vector<UserObjectName> >("uo_slip_rates")[i]);
  ...
  {
    _uo_slip_resistances[i] = &getUserObjectByName<anaCrystalPlasticitySlipResistanceGSS>(parameters.get<std::vector<UserObjectName> >("uo_slip_resistances")[i]);
  ...
  {
    _uo_state_vars[i] = &getUserObjectByName<anaCrystalPlasticityStateVariable>(parameters.get<std::vector<UserObjectName> >("uo_state_vars")[i]);
  ...
  {
    _uo_state_var_evol_rate_comps[i] = &getUserObjectByName<anaCrystalPlasticityStateVarRateComponentGSS>(parameters.get<std::vector<UserObjectName> >("uo_state_var_evol_rate_comps")[i]);
...
void anaFiniteStrainUObasedCP::initQpStatefulProperties()
{
  for (unsigned int i = 0; i < _num_uo_slip_rates; ++i)
  {
 ...
 */
void anaFiniteStrainUObasedCP::computeQpStress()
{
...
void
anaFiniteStrainUObasedCP::preSolveQp()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
anaFiniteStrainUObasedCP::solveQp()
{
  preSolveStatevar();
  solveStatevar();
  if (_err_tol)
...
void
anaFiniteStrainUObasedCP::postSolveQp()
{
...
void
anaFiniteStrainUObasedCP::preSolveStatevar()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
anaFiniteStrainUObasedCP::solveStatevar()
{
  unsigned int iterg;
  bool iter_flag = true;
...
bool
anaFiniteStrainUObasedCP::isStateVariablesConverged()
{
  Real diff;
...
void
anaFiniteStrainUObasedCP::postSolveStatevar()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
    (*_mat_prop_state_vars_old[i])[_qp] = (*_mat_prop_state_vars[i])[_qp];
...
void
anaFiniteStrainUObasedCP::preSolveStress()
{
}

void
anaFiniteStrainUObasedCP::solveStress()
{
  unsigned int iter = 0;
  RankTwoTensor dpk2;
...
void
anaFiniteStrainUObasedCP::postSolveStress()
{
  _fp[_qp] = _fp_inv.inverse();
}

void
anaFiniteStrainUObasedCP::updateSlipSystemResistanceAndStateVariable()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
anaFiniteStrainUObasedCP::calcResidJacob()
{
  calcResidual();
  if (_err_tol)
    return;
...
void
anaFiniteStrainUObasedCP::getSlipRates()
{
  for (unsigned int i = 0; i < _num_uo_slip_rates; ++i)
  {
...
void
anaFiniteStrainUObasedCP::calcResidual()
{
...
void
anaFiniteStrainUObasedCP::calcJacobian()
{
...
void
anaFiniteStrainUObasedCP::calcTangentModuli()
{
...
void
anaFiniteStrainUObasedCP::elastoPlasticTangentModuli()
{
...
void
anaFiniteStrainUObasedCP::elasticTangentModuli()
{
...
bool
anaFiniteStrainUObasedCP::lineSearchUpdate(const Real rnorm_prev, const RankTwoTensor dpk2)
{
