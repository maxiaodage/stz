...
stzchisource::stzchisource(const InputParameters & parameters) :
...
Real
stzchisource::computeQpResidual()
{


  Real s = _s[_qp];
  Real s0= _s1l*_p+_s2l*_p*_u[_qp];
  Real T_current = std::tanh(_e0*s/(_p*_ez*_u[_qp]));
  ...
  Real bigc=2.0*_e0*s0/(_p*tscale*_e1)*std::exp(-1.0/_u[_qp])*mechnoise;
...
//  eps = eps0*tanh((t_fric*plasticStrainRate).^2);
 //2.0*e0*s0./(p-alpha*pf).*exp(-1.0./chi)/(tscale*e1)
  ...
  return -_test[_i][_qp]*(2.0*_e0*s0/(_p*tscale*_e1)*std::exp(-1.0/_u[_qp])*(mechnoise*(1.0-_u[_qp]/chihat)-eps*_u[_qp]/chihat));
