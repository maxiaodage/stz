...
stzStressDivergenceTensors::stzStressDivergenceTensors(const InputParameters & parameters) :
...
Real
stzStressDivergenceTensors::computeQpResidual()
{
  return _stress[_qp].row(_component) * _grad_test[_i][_qp];
}

void
stzStressDivergenceTensors::computeJacobian()
{
  if (_use_finite_deform_jacobian)
  {
...
void
stzStressDivergenceTensors::computeOffDiagJacobian(unsigned int jvar)
{
  if (_use_finite_deform_jacobian)
  {
    _finite_deform_Jacobian_mult.resize(_qrule->n_points());
...
Real
stzStressDivergenceTensors::computeQpJacobian()
{
  if (_use_finite_deform_jacobian)
...
Real
stzStressDivergenceTensors::computeQpOffDiagJacobian(unsigned int jvar)
{
...
void
stzStressDivergenceTensors::computeFiniteDeformJacobian()
{
