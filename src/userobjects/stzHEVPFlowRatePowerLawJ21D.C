...
InputParameters validParams<stzHEVPFlowRatePowerLawJ21D>()
{
 ...
 stzHEVPFlowRatePowerLawJ21D::stzHEVPFlowRatePowerLawJ21D(const InputParameters & parameters) :
 ...
 bool
 stzHEVPFlowRatePowerLawJ21D::computeValue(unsigned int qp, Real & val) const
 {
...
// #endif
 ...
 bool
 stzHEVPFlowRatePowerLawJ21D::computeDirection(unsigned int qp, RankTwoTensor & val) const
 {
   RankTwoTensor pk2_dev = computePK2Deviatoric(_pk2[qp], _ce[qp]);
   Real eqv_stress = computeEqvStress(pk2_dev, _ce[qp]);
   ...
   if (eqv_stress > _strength[qp])
     {//   val = 1.5/eqv_stress * _ce[qp] * pk2_dev ;
      ...
      //            <<_pk2[qp](2,0)<<","<<_pk2[qp](2,1)<<","<<_pk2[qp](2,2)<<";"<<"\n";
 ...
 bool
  stzHEVPFlowRatePowerLawJ21D::computeDerivative(unsigned int qp, const std::string & coupled_var_name, Real & val) const
 {
   val = 0.0;
 ...
 bool
 stzHEVPFlowRatePowerLawJ21D::computeTensorDerivative(unsigned int qp, const std::string & coupled_var_name, RankTwoTensor & val) const
 {
   val.zero();
...
RankTwoTensor
 stzHEVPFlowRatePowerLawJ21D::computePK2Deviatoric(const RankTwoTensor & pk2, const RankTwoTensor & ce) const
{
  return pk2 - (pk2.doubleContraction(ce) * ce.inverse())/3.0;
}

Real
stzHEVPFlowRatePowerLawJ21D::computeEqvStress(const RankTwoTensor & pk2_dev, const RankTwoTensor & ce) const
{
