...
InputParameters validParams<stzcHEVPFlowRatePowerLawJ2>()
{
 ...
 stzcHEVPFlowRatePowerLawJ2::stzcHEVPFlowRatePowerLawJ2(const InputParameters & parameters) :
 ...
 bool
 stzcHEVPFlowRatePowerLawJ2::computeValue(unsigned int qp, Real & val) const
 {
  ...
  //  Real _t_taunew = _agrain[qp]/std::sqrt(_grain_pressure/_rho);
   ...
   //  val = std::pow(eqv_stress/_strength[qp], _flow_rate_exponent) * _ref_flow_rate*std::exp(_grain_pressure/_grain_pressure)*(eqv_stress>_strength[qp]);
    ...
    val = 1.0/_t_taunew*std::exp(-1.0/_chiv)*std::exp(eqv_stress/_pressure_eff/_chiv)*(1.0-_strength[qp]/eqv_stress);
 ...
 bool
 stzcHEVPFlowRatePowerLawJ2::computeDirection(unsigned int qp, RankTwoTensor & val) const
 {
   RankTwoTensor pk2_dev = computePK2Deviatoric(_pk2[qp], _ce[qp]);
   Real eqv_stress = computeEqvStress(pk2_dev, _ce[qp]);
 ...
 bool
  stzcHEVPFlowRatePowerLawJ2::computeDerivative(unsigned int qp, const std::string & coupled_var_name, Real & val) const
 {
   val = 0.0;
     ...
     if(eqv_stress>=_strength[qp])
    {
      Real _chiv=0.08;
         ...
         //   val=1.0/_t_taunew*std::exp(-1.0/_chiv[qp])*std::exp(eqv_stress/_grain_pressure/_chiv[qp])*(-1.0/eqv_stress);
           val=1.0/_t_taunew*std::exp(-1.0/_chiv)*std::exp(eqv_stress/_pressure_eff/_chiv)*(-1.0/eqv_stress);
    }
 ...
 bool
 stzcHEVPFlowRatePowerLawJ2::computeTensorDerivative(unsigned int qp, const std::string & coupled_var_name, RankTwoTensor & val) const
 {
   val.zero();
...
{
  Real _chiv=0.08;
  dflowrate_dseqv = 1/_t_taunew*std::exp(-1/_chiv)*std::exp(eqv_stress/_pressure_eff/_chiv)* \
(1.0/(_pressure_eff*_chiv)*(1.0-_strength[qp]/eqv_stress)+_strength[qp]/(eqv_stress*eqv_stress));
}
...
RankTwoTensor
 stzcHEVPFlowRatePowerLawJ2::computePK2Deviatoric(const RankTwoTensor & pk2, const RankTwoTensor & ce) const
{
  return pk2 - (pk2.doubleContraction(ce) * ce.inverse())/3.0;
}

Real
stzcHEVPFlowRatePowerLawJ2::computeEqvStress(const RankTwoTensor & pk2_dev, const RankTwoTensor & ce) const
{
